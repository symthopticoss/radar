...
sudo mkdir -p /etc/radar
cd /etc/radar

# Get strategies
git clone https://github.com/symthopticoss/radar/tree/main/strategies

# Generate the config.json file with the provided parameters
sudo bash -c "cat > /etc/radar/config.json << EOL
{
...
echo "Installing Radar v1.0.0..."
curl -sL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt update
sudo apt-get install -y nodejs
sudo curl -o /etc/radar/radar.js https://raw.githubusercontent.com/symthopticoss/radar/refs/heads/main/index.js
cd /etc/sryden
sudo npm init -y
...
sudo pm2 start /etc/radar/radar.js --name "radar"
sudo pm2 save
sudo pm2 startup
